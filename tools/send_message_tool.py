...
async def _send_signal(extra, chat_id, message, media_files=None):
    """Send through the gateway's one persistent signal-ts connection.

    A second, one-shot Signal client cannot safely share linked-device state
    with the gateway and would reintroduce connection setup latency.  Proactive
    sends therefore require the live gateway adapter; cron ``session: main``
    executions naturally satisfy that contract.
    """
    del extra  # Runtime paths belong to the connected adapter, not this tool.
    try:
        from gateway.config import Platform
        from gateway.run import _gateway_runner_ref

        runner = _gateway_runner_ref()
        adapter = runner.adapters.get(Platform.SIGNAL) if runner is not None else None
        if adapter is None:
            return {
                "error": (
                    "Signal requires the live gateway's persistent signal-ts adapter. "
                    "Start the gateway or run this scheduled action with session: main."
                )
            }
        ...
        if media_files and not valid_media and not message.strip():
            return _error("Signal send failed: no valid media files")
        if valid_media:
            sent = await adapter.send_files(chat_id, valid_media, caption=message)
        else:
            sent = await adapter.send(chat_id=chat_id, content=message)
        if not sent.success:
            return _error(f"Signal send failed: {sent.error or 'unknown adapter error'}")
