...
def _build_child_agent(
    ...
    # native Claude CLI runtime; ``override_api_mode`` disambiguates them.
    ...
    if effective_api_mode == "claude_cli":
    ...
    if override_acp_command and effective_api_mode != "claude_cli":
        # A non-Claude command override is the legacy Copilot ACP path, whose
        # client selection is provider-driven.  Native Claude CLI also carries
        # its executable through ``acp_command`` for compatibility, but its
        # already-resolved anthropic/claude_cli identity must remain intact.
    ...
    with delegated_child_context():
        ...
        child._claude_cli_persistent_binding = False
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    # missing credentials disables explicitly configured Claude CLI delegates.
    if not api_key and api_mode != "claude_cli":
        raise ValueError(
