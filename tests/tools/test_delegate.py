...
class TestDelegationCredentialResolution(unittest.TestCase):
    ...
    @patch("hermes_cli.runtime_provider.resolve_runtime_provider")
    def test_subscription_claude_cli_provider_needs_no_api_key(self, mock_resolve):
        mock_resolve.return_value = {
            "provider": "anthropic",
            "model": "claude-opus-4-6",
            "base_url": "",
        ...
        self.assertEqual(creds["api_mode"], "claude_cli")
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_default_child_inherits_subscription_claude_cli_runtime(
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            self.assertEqual(kwargs["api_mode"], "claude_cli")
            self.assertFalse(kwargs["api_key"])
            self.assertFalse(kwargs["base_url"])
            self.assertIs(mock_child._claude_cli_persistent_binding, False)

    @patch("tools.delegate_tool._load_config")
    @patch("tools.delegate_tool._resolve_delegation_credentials")
    @patch("shutil.which", return_value="/opt/claude")
    def test_configured_subscription_claude_cli_command_keeps_native_runtime(
