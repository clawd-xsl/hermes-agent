...
def test_probe_credentials_allows_claude_agent_sdk_keychain_runtime():
    agent = types.SimpleNamespace(
        api_key="", provider="anthropic", api_mode="claude_agent_sdk"
    )
...
def test_setup_runtime_check_allows_claude_agent_sdk_keychain_runtime(monkeypatch):
    monkeypatch.setattr("hermes_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "hermes_cli.runtime_provider.resolve_runtime_provider",
        lambda requested=None: {
...
def test_claude_agent_sdk_partial_compress_targets_native_history_with_recent_window():
    calls = []
