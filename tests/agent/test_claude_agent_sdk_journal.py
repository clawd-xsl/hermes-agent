...
def test_completed_effect_is_replayed_after_restart(tmp_path):
    first = ClaudeToolJournal("owner", root=tmp_path)
    assert first.claim_batch([_intent()])[0].disposition == "execute"
    first.complete([_result()])

    recovered = ClaudeToolJournal("owner", root=tmp_path)
...
def test_concurrent_journal_instances_cannot_both_claim_one_effect(tmp_path):
    first = ClaudeToolJournal("owner", root=tmp_path)
    second = ClaudeToolJournal("owner", root=tmp_path)
    ...
    threads = [
        threading.Thread(target=claim, args=(journal,)) for journal in (first, second)
    ]
...
def test_batch_claim_is_one_durable_transition(tmp_path, monkeypatch):
    journal = ClaudeToolJournal("owner", root=tmp_path)
...
def test_reused_tool_id_with_different_input_fails_closed(tmp_path):
    ...
    with pytest.raises(ClaudeToolJournalError, match="different tool intent"):
...
def test_corrupt_journal_fails_closed(tmp_path):
    ...
    with pytest.raises(ClaudeToolJournalError, match="unreadable"):
        ClaudeToolJournal("owner", root=tmp_path)
