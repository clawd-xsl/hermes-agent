...
def test_tool_proposal_mode_never_invokes_agent_executor():
    ...
    loopback = ClaudeToolLoopback(ProposalAgent(), serve=False)
    try:
        loopback.begin_turn(
            task_id="proposal",
...
def test_background_review_tool_policy_crosses_loopback_server_thread(
    monkeypatch,
):
    """The turn's logical policy must survive the MCP server thread hop."""
...
def test_loopback_persists_intent_before_standard_executor_and_result_after(
    monkeypatch,
):
    calls = []
    projection_order = []
    ...
    try:
        ...
        assert loopback.reconcile_authoritative_projection(
            [
                {
                    "role": "assistant",
                    "content": None,
...
def test_native_projection_repairs_duplicate_tool_ids_without_losing_calls():
    ...
    try:
        ...
        projected = loopback.reconcile_authoritative_projection(
            [
                {
                    "role": "assistant",
                    "content": None,
...
def test_authoritative_batch_does_not_execute_duplicate_calls_twice():
    ...
    try:
        ...
        first = loopback._call_tool(
            {"name": "echo", "arguments": {"value": "same"}}
        )
        second = loopback._call_tool(
            {"name": "echo", "arguments": {"value": "same"}}
        )
...
def test_tool_side_effect_is_refused_when_intent_persistence_fails(monkeypatch):
    ...
    try:
        ...
        assert response["error"]["type"] == "RuntimeError"
        assert "refusing to execute" in response["error"]["message"]
        assert executed == []
...
def test_loopback_refuses_followup_side_effect_after_guardrail_halt():
    class _Decision:
        pass
...
def test_authoritative_housekeeping_batch_mutes_post_answer_tool_noise():
    ...
    try:
        ...
        loopback.reconcile_authoritative_projection(
...
def test_tool_batch_wait_inherits_native_turn_timeout():
    agent = _Agent()
    agent._claude_cli_session = SimpleNamespace(turn_timeout=1_800.0)
    loopback = ClaudeToolLoopback(agent, serve=False)
    try:
        assert loopback._tool_batch_timeout() == 1_800.0
    finally:
        loopback.close()
