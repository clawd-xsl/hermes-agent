...
from agent.claude_agent_sdk_runtime import (
    ...
    run_claude_agent_sdk_auxiliary_completion,
...
def test_native_turn_signature_tracks_host_control_and_history_state():
    ...
    for field, value in (
        ...
        changed = ClaudeAgentSdkTurnResult(**vars(base))
...
def test_native_display_error_is_bounded_and_force_redacted():
    ...
    assert rendered.startswith("Claude Agent SDK turn failed:")
...
def test_one_shot_agent_uses_nonpersistent_native_history(monkeypatch):
    ...
    agent = SimpleNamespace(
        ...
        _claude_agent_sdk_persistent_binding=False,
        _claude_agent_sdk_command="claude",
...
def test_reasoning_effort_maps_hermes_edges_to_claude_agent_sdk_levels():
...
def test_auxiliary_completion_is_isolated_toolless_and_closes(monkeypatch):
    ...
    monkeypatch.setattr("agent.claude_agent_sdk_runtime._new_transient_session", _new)
...
def test_manual_compaction_reuses_effective_persistent_session_prompt(monkeypatch):
    ...
    result = compact_claude_agent_sdk_history(agent, focus_topic="keep decisions")
