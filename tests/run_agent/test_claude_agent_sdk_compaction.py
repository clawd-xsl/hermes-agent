...
class _Agent:
    def __init__(self) -> None:
        self.api_mode = "claude_agent_sdk"
...
def test_manual_compress_delegates_to_native_boundary_and_runs_hermes_lifecycle(
    ...
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime.compact_claude_agent_sdk_history", compact
    )
    monkeypatch.setattr(
...
def test_failed_native_compaction_is_an_explicit_abort(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime.compact_claude_agent_sdk_history",
...
def test_explicit_external_engine_selection_wins_over_native_default():
    ...
    assert claude_agent_sdk_owns_context_compaction(agent) is False
