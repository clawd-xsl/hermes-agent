...
def test_claude_agent_sdk_agent_does_not_construct_http_inference_client():
    agent = _make_agent()
    try:
...
def test_main_native_structured_output_survives_turn_result(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime._get_session",
...
def test_native_session_uses_claude_specific_command_and_args(monkeypatch):
    from agent.claude_agent_sdk_runtime import _new_transient_session
    ...
    monkeypatch.setattr(
        "agent.transports.claude_agent_sdk_session.ClaudeAgentSdkToolBridge",
        _Loopback,
    )
    monkeypatch.setattr("agent.claude_agent_sdk_runtime._runtime_config", lambda: {})
...
def test_moa_turn_retires_private_guidance_at_turn_boundary(monkeypatch):
    ...
    monkeypatch.setattr("agent.claude_agent_sdk_runtime._get_session", fake_get_session)
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime.forget_claude_agent_sdk_binding",
        lambda _owner: None,
    )
    monkeypatch.setattr(
    ...
    try:
        ...
        assert agent._claude_agent_sdk_session is None
...
def test_live_model_switch_can_enter_subscription_runtime_without_http_auth():
    ...
    try:
        ...
        assert agent._primary_runtime["api_mode"] == "claude_agent_sdk"
        assert agent._primary_runtime["client_kwargs"] == {}
        assert agent._claude_agent_sdk_command is None
        assert agent._claude_agent_sdk_args == []
    finally:
        agent.close()
...
def test_live_model_switch_away_from_subscription_retires_native_session():
    agent = _make_agent()
    native = _FakeSession()
    agent._claude_agent_sdk_session = native
    try:
        with (
            patch("agent.credential_pool.load_pool", return_value=None),
        ...
        with (
            patch.object(
                ...
                side_effect=AssertionError(
                    "restoring claude_agent_sdk must not construct OpenAI"
                ),
            ),
            patch("agent.credential_pool.load_pool", return_value=None),
        ):
...
def test_conversation_loop_uses_native_runtime_and_persists_projection(monkeypatch):
    ...
    try:
        ...
        assert result["claude_agent_sdk_latency_ms"] == {"first_text": 12, "total": 30}
...
def test_native_turn_honors_interrupt_that_precedes_child_dispatch(monkeypatch):
    ...
    monkeypatch.setattr("agent.claude_agent_sdk_runtime._get_session", get_session)
...
def test_native_budget_exhaustion_gets_isolated_toolless_summary(monkeypatch):
    ...
    monkeypatch.setattr("agent.claude_agent_sdk_runtime._get_session", get_session)
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime._new_transient_session",
        new_transient,
    )
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime._invalidate_persistent_native_history",
        lambda bound_agent, _session: setattr(
            bound_agent, "_claude_agent_sdk_session", None
        ),
    )
    with patch.object(agent, "_sync_external_memory_for_turn", return_value=None):
...
def test_native_runtime_fires_request_scoped_observer_hooks(monkeypatch):
    ...
    try:
        ...
        assert pre[0]["api_mode"] == post[0]["api_mode"] == "claude_agent_sdk"
        assert pre[0]["request"]["body"]["transport"] == "claude_agent_sdk"
...
def test_changed_context_selection_uses_isolated_one_turn_native_child(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime._select_native_request_context",
...
def test_automatic_native_compaction_runs_hermes_boundary_and_carries_memory(
    ...
    try:
        ...
        assert agent._claude_agent_sdk_pending_compaction_context == ""
...
def test_native_execution_middleware_mutation_uses_isolated_child(monkeypatch):
    ...
    def get_session(bound_agent, **_kwargs):
        bound_agent._claude_agent_sdk_session = persistent
        return persistent
...
def test_native_physical_attempt_runs_through_relay(monkeypatch):
    ...
    try:
        ...
        assert relay_calls[0][1]["metadata"]["api_mode"] == "claude_agent_sdk"
...
def test_native_final_answer_gates_continue_on_same_warm_session(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime._native_continuation_nudge",
        continuation,
    )
    monkeypatch.setattr(
        "hermes_cli.middleware.apply_llm_request_middleware",
...
def test_native_empty_response_retries_then_returns_visible_answer(monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime._native_empty_retry_wait",
...
def test_native_max_tokens_continuation_is_bounded_and_returns_partial(
    ...
    try:
        ...
        assert result["turn_exit_reason"] == "claude_agent_sdk_output_truncated"
        assert result["error"] == (
...
def test_native_thinking_budget_exhaustion_does_not_retry_continuation(
    ...
    try:
        ...
        assert result["turn_exit_reason"] == (
            "claude_agent_sdk_thinking_budget_exhausted"
        )
...
def test_native_transport_failure_restarts_once_before_fallback(monkeypatch):
    ...
    def get_session(bound_agent, **_kwargs):
        ...
        bound_agent._claude_agent_sdk_session = session
        return session
...
def test_native_terminal_error_is_redacted_before_main_history(monkeypatch):
    ...
    try:
        assert result["failed"] is True
        assert result["final_response"].startswith("Claude Agent SDK turn failed:")
...
def test_native_image_rejection_retries_on_clean_session_with_api_sidecar(
    ...
    monkeypatch.setattr("agent.claude_agent_sdk_runtime._get_session", get_session)
    monkeypatch.setattr(
        "agent.claude_agent_sdk_runtime._shrink_native_history_images", shrink
    )
    user_content = [
        {"type": "text", "text": "inspect"},
        {
