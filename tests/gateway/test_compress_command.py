...
@pytest.mark.asyncio
async def test_compress_command_allows_claude_cli_subscription_without_api_key():
    history = _make_history()
    runner = _make_runner(history)
    ...
    agent_instance.shutdown_memory_provider = MagicMock()
    agent_instance.close = MagicMock()
    agent_instance.api_mode = "claude_cli"
    agent_instance._cached_system_prompt = ""
    agent_instance.tools = None
    agent_instance.session_id = "sess-1"
    ...
    assert agent_instance._claude_cli_session is None


@pytest.mark.asyncio
async def test_compress_here_preserves_external_engine_semantics_on_claude_cli():
    history = _make_history()
    runner = _make_runner(history)
    entry = runner.session_store.get_or_create_session.return_value
    runner._async_session_store = SimpleNamespace(
    ...
    with (
        ...
        patch("run_agent.AIAgent", return_value=agent_instance),
        patch("agent.model_metadata.estimate_request_tokens_rough", return_value=100),
        patch("agent.claude_cli_runtime.detach_claude_cli_session") as detach,
    ):
