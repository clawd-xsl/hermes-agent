...
@pytest.mark.asyncio
async def test_session_hygiene_uses_native_compaction_for_keychain_claude_agent_sdk(
    monkeypatch, tmp_path
):
    ...
    class FakeNativeCompressAgent:
        ...
        def __init__(self, **kwargs):
            ...
            self._claude_agent_sdk_session = object()
            self.context_compressor = SimpleNamespace(
                bind_session_state=MagicMock(),
                _last_compress_aborted=False,
                _last_aux_model_failure_model=None,
        ...
        def _compress_context(self, messages, *_args, **kwargs):
            assert self.api_mode == "claude_agent_sdk"
    ...
    assert FakeNativeCompressAgent.last_instance._claude_agent_sdk_session is None
