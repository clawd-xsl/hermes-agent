...
claude_agent_sdk`. An existing Anthropic Claude model is preserved; otherwise the
preset uses `claude-sonnet-4-6`. A non-Claude Hermes model is treated as a
conflict and is never replaced without `--overwrite`.

This runtime keeps one Claude Agent SDK client and its native session alive
across turns and gateway restarts. Claude owns native history rollover and
compaction, so Hermes does not add a second session summary. The SDK and its
version-matched bundled Claude Code runtime are installed lazily on first use;
authenticate through an existing Claude Code login or an official
`CLAUDE_CODE_OAUTH_TOKEN`. Agent SDK calls use Anthropic's separate Agent SDK
credit/usage-credit pool rather than the interactive Claude subscription quota.
See [Persistent Claude Agent SDK Runtime](/user-guide/features/claude-agent-sdk-runtime).
...
4. **Check Claude authentication** — send one test message after migration. The Agent SDK uses an existing Claude Code login or an explicitly exported official `CLAUDE_CODE_OAUTH_TOKEN`; it does not need the Hermes MCP extra.
