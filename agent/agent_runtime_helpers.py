...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        if agent.api_mode == "claude_agent_sdk":
            ...
            agent._claude_agent_sdk_command = rt.get("claude_agent_sdk_command")
            agent._claude_agent_sdk_args = list(rt.get("claude_agent_sdk_args") or [])
    ...
    *,
    ...
    try:
        ...
        if getattr(agent, "api_mode", None) == "claude_agent_sdk":
            request_url = "claude-agent-sdk://local/stream-json"
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    _snapshot = {
        ...
        for name in (
            ...
            "_anthropic_base_url",
            "_is_anthropic_oauth",
            "_claude_agent_sdk_command",
            "_claude_agent_sdk_args",
    ...
    try:
        ...
        if api_mode == "claude_agent_sdk" and _snapshot.get("api_mode") != "claude_agent_sdk":
            # A live switch/fallback into Claude uses model.claude_agent_sdk config.
            # Do not inherit another subprocess transport's command/args.
            agent._claude_agent_sdk_command = None
    ...
    # session. This covers both leaving Claude Agent SDK and changing native models.
    if _snapshot.get("api_mode") == "claude_agent_sdk" or api_mode == "claude_agent_sdk":
        try:
            ...
            logger.debug(
                "switch_model: failed to retire prior Claude Agent SDK session",
                exc_info=True,
            )
