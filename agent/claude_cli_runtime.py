...
def _native_unsupported_request_controls(agent: Any) -> list[str]:
    ...
    if (
        getattr(agent, "_claude_cli_unsupported_controls_signature", None)
        == control_signature
    ):
        cached = getattr(agent, "_claude_cli_unsupported_controls", None)
    ...
    agent._claude_cli_unsupported_controls_signature = control_signature
    agent._claude_cli_unsupported_controls = controls
    if controls:
        message = (
            "Persistent Claude CLI cannot forward API-wire controls: "
            + ", ".join(controls)
            + ". Claude low-latency mode is supported through Hermes /fast "
            "or model.claude_cli.fast_mode."
        )
...
def _native_timeouts(
    ...
    ``model.claude_cli.turn_timeout_seconds`` bounds Claude Code's complete
...
def _get_session(agent: Any, *, system_prompt: str) -> ClaudeCliSession:
    ...
    persistent_binding = (
        bool(getattr(agent, "_claude_cli_persistent_binding", True))
        and not getattr(agent, "_persist_disabled", False)
    )
    cfg = _runtime_config()
    command = str(
        getattr(agent, "_claude_cli_command", None)
        or cfg.get("command")
        or "claude"
    )
    configured_args = (
        getattr(agent, "_claude_cli_args", None) or cfg.get("args") or []
    )
...
def run_claude_cli_auxiliary_completion(
    *,
    messages: list[dict[str, Any]],
    model: str,
    ...
    if not isinstance(messages, list) or not messages:
        raise ValueError("Claude CLI auxiliary completion requires messages")
    ...
    if not conversation or conversation[-1].get("role") != "user":
        raise ValueError(
            "Claude CLI auxiliary completion requires a final user message"
        )
    ...
    if getattr(result, "interrupted", False):
        raise RuntimeError("Claude CLI auxiliary completion was interrupted")
    ...
    if (
        ...
        raise RuntimeError("Claude CLI auxiliary completion returned empty output")
    return result
...
def _invalidate_persistent_native_history(
    agent: Any, previous_session: Optional[ClaudeCliSession]
) -> None:
    """Drop a warm thread that missed an isolated selected-context turn."""
    owner_key = _owner_key(agent)
    if previous_session is not None:
        _retire_session(agent, previous_session)
    forget_claude_cli_binding(owner_key)
    if getattr(agent, "_claude_cli_session", None) is previous_session:
        agent._claude_cli_session = None
...
def detach_claude_cli_session(agent: Any) -> None:
    ...
    """
    if getattr(agent, "_claude_cli_session", None) is not None:
        agent._claude_cli_session = None
...
def compact_claude_cli_history(
    agent: Any,
    *,
...
_PENDING_NATIVE_COMPACTION_CONTEXT = "_claude_cli_pending_compaction_context"
_STAMPED_NATIVE_COMPACTION_CONTEXT = "_claude_cli_compaction_context"
...
def stamp_native_compaction_context(
    ...
    native continuations call it from ``run_claude_cli_turn``.  The private
...
def _coerce_native_turn_result(value: Any) -> ClaudeCliTurnResult:
    """Normalize Relay/middleware JSON projections back to the native result."""
    if isinstance(value, ClaudeCliTurnResult):
        return value
    try:
    ...
    if not isinstance(payload, dict) or "projected_messages" not in payload:
        raise TypeError(
            "Claude LLM execution middleware must return ClaudeCliTurnResult"
        )
    allowed = ClaudeCliTurnResult.__dataclass_fields__
...
def _invoke_native_pre_request_hook(
    ...
    *,
    ...
    try:
        ...
        logger.debug("Claude CLI pre_api_request hook failed", exc_info=True)
...
def _invoke_native_post_request_hook(
    ...
    *,
    ...
    try:
        ...
        logger.debug("Claude CLI post_api_request hook failed", exc_info=True)
...
def _invoke_native_post_iteration_hook(
    ...
    *,
    ...
    try:
        ...
        response = agent._sanitize_hook_payload(
            {
                ...
                "native_session_id": getattr(
                    getattr(agent, "_claude_cli_session", None),
                    "native_session_id",
                    None,
                ),
            }
        ...
        logger.debug(
            "Claude CLI per-iteration post_api_request hook failed",
            exc_info=True,
        )
...
def _native_empty_retry_wait(agent: Any, attempt: int) -> bool:
    ...
    agent._buffer_status(
        f"⚠️ Empty response from Claude CLI — retrying "
...
def _native_iteration_limit_summary(
    ...
    *,
    ...
    previous_session = getattr(agent, "_claude_cli_session", None)
    ...
    agent._claude_cli_session = summary_session
    try:
        ...
        agent._claude_cli_session = previous_session
...
def _native_display_error(agent: Any, error: Any) -> str:
    ...
    raw = str(error or "Unknown Claude CLI error")
    try:
    ...
    try:
        ...
        summary = str(summary)
    return "Claude CLI turn failed: " + summary.strip()
...
def _sanitize_native_terminal_error_surface(agent: Any, turn: Any) -> None:
    ...
    Some Claude CLI versions repeat an error result as both ``final_text`` and
...
def _native_fallback_handoff(
    ...
    *,
    ...
    try:
        ...
        logger.warning("Claude CLI fallback activation failed", exc_info=True)
        return None
    if not activated:
        return None
    ...
    return {
        "_claude_cli_continue_standard_loop": True,
...
def run_claude_cli_turn(
    ...
    *,
    ...
    api_request_id = f"{turn_id}:claude-cli:{uuid.uuid4().hex[:12]}"
    ...
    if middleware_request.get("transport", "claude_cli") != "claude_cli":
        ...
        middleware_request["transport"] = "claude_cli"
    ...
    session: Optional[ClaudeCliSession] = None
    previous_session = getattr(agent, "_claude_cli_session", None)
    ...
    try:
        ...
        def _perform_native_physical_request(payload: dict[str, Any]) -> Any:
            ...
            if payload != middleware_request:
                ...
                if payload.get("transport") != "claude_cli":
                    raise RuntimeError(
            ...
            if callable(touch_activity):
                touch_activity(
                    f"starting Claude CLI request #{_api_calls_before + 1}"
                )
            agent._api_call_count = _api_calls_before + 1
        ...
        try:
            ...
            logger.debug(
                "Claude CLI Relay logical-call completion failed",
                exc_info=True,
            )
        ...
        try:
            agent._invoke_api_request_error_hook(
                ...
                reason=(
                    ...
                    or "claude_cli_startup_error"
                ),
            )
        except Exception:
            logger.debug("Claude CLI api_request_error hook failed", exc_info=True)
        ...
        if (
            ...
            agent._buffer_status(
                "⚠️ Claude CLI transport stopped before a response — "
                "restarting it once"
            )
        ...
        result = finalize_turn(
            ...
            _should_review_memory=should_review_memory,
            _turn_exit_reason="claude_cli_startup_error",
        )
    ...
    if callable(touch_activity):
        touch_activity(
            "Claude CLI request completed"
            if not turn.error
            else "Claude CLI request ended with an error"
        )
    ...
    if (
        ...
        try:
            ...
            agent._invoke_api_request_error_hook(
                ...
                error_type="ClaudeCliRuntimeError",
    ...
    if can_continue and native_stop_reason == "refusal":
        ...
        try:
            ...
            logger.debug(
                "Claude CLI content-policy error hook failed",
                exc_info=True,
            )
    ...
    if can_continue and not str(turn.final_text or "").strip():
        ...
        if recent_tool and not _post_tool_empty_retried:
            ...
            agent._buffer_status(
                "⚠️ Claude CLI returned empty after tool calls — "
    ...
    if guardrail is not None:
        ...
        exit_reason = "claude_cli_budget_exhausted"
    elif interrupted:
        ...
        exit_reason = "claude_cli_thinking_budget_exhausted"
    elif length_continuation_exhausted:
        exit_reason = "claude_cli_output_truncated"
    elif content_policy_blocked:
        exit_reason = "content_policy_blocked"
    elif failed:
        exit_reason = "claude_cli_error"
    else:
        exit_reason = "text_response(claude_cli)"
