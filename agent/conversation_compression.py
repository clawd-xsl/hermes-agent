...
def claude_agent_sdk_owns_context_compaction(agent: Any) -> bool:
    """Whether Claude's native history owns this agent's compaction policy.

    The persistent Claude Agent SDK transport owns the *physical* conversation, so
...
def compress_context(
    ...
    *,
    ...
    # With the built-in compressor, Claude Agent SDK owns the native conversation
    ...
    if claude_agent_sdk_owns_context_compaction(agent):
        if force or native_trigger_source:
            return _compress_context_via_claude_agent_sdk(
                agent,
                messages,
                system_message,
        ...
        logger.info(
            "Claude Agent SDK owns automatic native context compaction; Hermes "
...
def _compress_context_via_claude_agent_sdk(
    agent: Any,
    messages: list,
    ...
    try:
        if commit_fence is not None:
            ...
            if not commit_entered:
                ...
                _emit_compaction_done(agent)
                return messages, existing_prompt
        from agent.claude_agent_sdk_runtime import compact_claude_agent_sdk_history

        result = compact_claude_agent_sdk_history(agent, focus_topic=native_focus)
