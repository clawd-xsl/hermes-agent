...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    if explicit_fb_api_mode not in {
        ...
        return agent._try_activate_fallback(reason)
    is_claude_agent_sdk_fallback = explicit_fb_api_mode == "claude_agent_sdk"
    if is_claude_agent_sdk_fallback and fb_provider != "anthropic":
        logger.warning(
            "Fallback skip: claude_agent_sdk is only valid for provider=anthropic "
            "(got %s/%s)",
            fb_provider,
            fb_model,
        )
    ...
    fb_ident = BackendIdentity.build(
        ...
        # explicit local endpoint so HTTP -> Claude Agent SDK remains a viable
        # fallback while Claude Agent SDK -> the same Claude Agent SDK still deduplicates.
        base_url=(
            "claude-agent-sdk://local"
            if is_claude_agent_sdk_fallback
    ...
    try:
        ...
        fb_client = None
        if not is_claude_agent_sdk_fallback:
        ...
        # Claude's subscription runtime intentionally has no HTTP endpoint or
        # client; its explicit api_mode remains authoritative below.
        fb_base_url = "" if is_claude_agent_sdk_fallback else str(fb_client.base_url)
