...
class _ClaudeCliCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from agent.claude_cli_runtime import run_claude_cli_auxiliary_completion
...
class ClaudeCliAuxiliaryClient:
    ...
    def __init__(self, model: str):
        self.chat = _ClaudeCliChatShim(_ClaudeCliCompletionsAdapter(model))
        self.api_key = ""
...
class AsyncClaudeCliAuxiliaryClient:
    def __init__(self, sync_wrapper: "ClaudeCliAuxiliaryClient"):
        self.chat = _AsyncClaudeCliChatShim(
            _AsyncClaudeCliCompletionsAdapter(sync_wrapper.chat.completions)
        )
        self.api_key = sync_wrapper.api_key
        self.base_url = sync_wrapper.base_url
...
def _read_main_api_mode() -> str:
    ...
    normal resolver. ``claude_cli`` is different: it is an explicit runtime
    ...
    try:
        ...
        if provider == "anthropic" and runtime == "claude_cli":
            return "claude_cli"
...
def resolve_provider_client(
    ...
    Args:
        ...
        api_mode: API mode override.  One of "chat_completions",
            "codex_responses", "anthropic_messages", "claude_cli", or None
    ...
    if (
        ...
        if main_provider == "anthropic" and main_api_mode == "claude_cli":
            api_mode = "claude_cli"
    ...
    # native conversation.
    if api_mode == "claude_cli" and provider == "anthropic":
        final_model = _normalize_resolved_model(model or _read_main_model(), provider)
        if not final_model:
            logger.warning(
                "resolve_provider_client: claude_cli requested without a model"
            )
            return None, None
        client = ClaudeCliAuxiliaryClient(final_model)
    ...
    if requested == "auto":
        ...
        if main_provider and main_provider not in {"auto", "", "moa"}:
            ...
            if main_provider == "nous":
                ...
                # Claude users must stay on claude_cli instead of silently
    ...
    *,
    ...
    if stream:
        ...
        if task == "moa_aggregator" and isinstance(
            client, (CodexAuxiliaryClient, ClaudeCliAuxiliaryClient)
        ):
