    ...
    *,
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        _native_runtime_auto = (
            claude_agent_sdk_owns_context_compaction(agent)
            or (
    ...
    if (
        ...
        if getattr(agent, "api_mode", None) == "claude_agent_sdk":
            _pending_native_context = str(
                getattr(agent, "_claude_agent_sdk_pending_compaction_context", "") or ""
            ).strip()
            if _pending_native_context:
                from agent.claude_agent_sdk_runtime import stamp_native_compaction_context
