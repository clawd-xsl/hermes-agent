...
def _run_main_session_job(
    ...
    *,
    ...
    try:
        # Preserve upstream monitor-mode semantics before waking the main
        # conversation. An unchanged source is a silent, zero-LLM tick; a
        # change becomes one-run context for the real main-session turn.
        from cron.monitor import check_monitor, job_has_monitor

        if job_has_monitor(job):
            monitor = check_monitor(job)
            if not monitor.ok:
                monitor_error = (
                    "## Monitor Source Error\n"
                    "The scheduled monitor source failed. Report this failure "
                    "to the user and help diagnose it.\n\n"
                    f"```\n{monitor.error}\n```"
                )
                extra_prompt = (
                    f"{monitor_error}\n\n{extra_prompt}"
                    if extra_prompt else monitor_error
                )
            elif not monitor.changed:
                output = (
                    f"# Cron Job: {job_name}\n\n"
                    f"**Job ID:** {job_id}\n"
                    f"**Run Time:** {run_time.strftime('%Y-%m-%d %H:%M:%S')}\n"
                    "**Session:** main\n"
                    "**Status:** no_change (main turn suppressed)\n"
                )
                return True, output, None
            elif monitor.context_block:
                extra_prompt = (
                    f"{monitor.context_block}\n\n{extra_prompt}"
                    if extra_prompt else monitor.context_block
                )
...
def run_one_job(
    ...
    try:
        ...
        if _session_mode == "main":
            try:
                _main_kwargs = {"loop": loop}
                if extra_prompt is not None:
                    _main_kwargs["extra_prompt"] = extra_prompt
                success, output, error = _run_main_session_job(job, **_main_kwargs)
            finally:
